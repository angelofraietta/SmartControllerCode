/*
  ...
  to the networking stack. The following code declares the BSD config:
   ...
   in the CS8900 device strucyure. You need to map your target IRQ to the
   CS8900 in the BSP driver.

   Note, the driver does not handle the different interrupt pins on the
   CS8900 device. If you need to handle different pins, please extend the
   device structure and add a field to configure this.
...
/*
 * WARNING: hack mode on. This is only while developing the code.
 */
#define KERNEL
