...
static const char* patch_depth_filename = "/mnt/hda/PDEPTH.txt";
...
void DiagTask(void)
{
  ...
  // get the patch depth
  FILE* f_depth = fopen(patch_depth_filename, "r");
	if (f_depth)
		{
			unsigned patch_depth;
			int num_fields = fscanf (f_depth, "%u", &patch_depth);
      if (num_fields < 1)
      {
        printf ("Unable to read max patch depth\r\n");
      }
      else
      {
        printf ("Set Max patch depth : %u\r\n", patch_depth);
        SetMaxPatchViewDepth(patch_depth);
      }
      fclose(f_depth);
    }
	RestoreNVRAM();
